...
def main():
    ...
    for party, count in counter.items():
        print(f"  {party}: {count} votes ({count/len(ballots)*100:.1f}%)")    # UK voter profiles based on demographic segments
    # E: expertise (political knowledge), P: participation (voting history),
    # D: decision quality (consistency), A: alignment (with general public interest), S: stake (impact on voter)
    profiles = [
        # Urban professionals
    ...
    yes_ids = ['urban_professionals', 'students', 'young_professionals', 'public_sector', 'first_time_voters']
    ...
    print(f"   YES: {result_equal['total_yes']:.3f} ({result_equal['total_yes']/(result_equal['total_yes']+result_equal['total_no'])*100:.1f}%)")
    print(f"   NO:  {result_equal['total_no']:.3f} ({result_equal['total_no']/(result_equal['total_yes']+result_equal['total_no'])*100:.1f}%)")
    
    print(f"\n2. Expertise-focused weights:")
    print(f"   Passed? {result_expert['passed']}")
    print(f"   YES: {result_expert['total_yes']:.3f} ({result_expert['total_yes']/(result_expert['total_yes']+result_expert['total_no'])*100:.1f}%)")
    print(f"   NO:  {result_expert['total_no']:.3f} ({result_expert['total_no']/(result_expert['total_yes']+result_expert['total_no'])*100:.1f}%)")
    
    print(f"\n3. Stake-focused weights:")
    print(f"   Passed? {result_stake['passed']}")
    print(f"   YES: {result_stake['total_yes']:.3f} ({result_stake['total_yes']/(result_stake['total_yes']+result_stake['total_no'])*100:.1f}%)")
    print(f"   NO:  {result_stake['total_no']:.3f} ({result_stake['total_no']/(result_stake['total_yes']+result_stake['total_no'])*100:.1f}%)")
    ...
    for round_detail in results.round_details:
        ...
        sorted_results = sorted(round_detail["tallies"].items(), key=lambda x: x[1], reverse=True)
        ...
        if "eliminated" in round_detail:
            ...
            eliminated_name = next((c.name for c in candidates if c.id == eliminated_id), eliminated_id)
    ...
    charts["Ranked Choice Results"] = create_ranked_choice_visualization(results, candidates)
